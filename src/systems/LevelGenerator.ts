...
export class LevelGenerator {
  ...
  private static generateBossDuelLevel(levelNum: number): LevelData {
    ...
    if (levelNum === 4) {
      ...
      const m1 = new Enemy(170, -1180, 54, 44, 'boss_minion', 5, 'DRONE SOLAIRE');
      const m2 = new Enemy(270, -1210, 58, 48, 'boss_minion', 5, 'DRONE COMMANDANT');
      const m3 = new Enemy(370, -1180, 54, 44, 'boss_minion', 5, 'DRONE SOLAIRE');
      enemies.push(m1, m2, m3);
    } else if (levelNum === 9) {
      // --- BETAPULSAR : Deux Générateurs Magnétiques (10 PV max pour les premiers) ---
      bossEnemy.isInvulnerable = true;

      const genLeft = new Enemy(95, -1250, 80, 80, 'shield_generator', 10, 'GÉNÉRATEUR ALPHA');
      genLeft.generatorSide = 'left';
      genLeft.targetBoss = bossEnemy;

      const genRight = new Enemy(445, -1250, 80, 80, 'shield_generator', 10, 'GÉNÉRATEUR BETA');
      ...
      const ancLeft = new Enemy(95, -1250, 85, 85, 'shield_generator', 50, 'ANCRE OMEGA 1');
      ancLeft.generatorSide = 'left';
      ancLeft.targetBoss = bossEnemy;

      const ancCenter = new Enemy(270, -1200, 90, 90, 'shield_generator', 50, 'NŒUD SINGULARITÉ');
      ancCenter.generatorSide = 'center';
      ancCenter.targetBoss = bossEnemy;

      const ancRight = new Enemy(445, -1250, 85, 85, 'shield_generator', 50, 'ANCRE OMEGA 2');
